...
SELECT MARK, color, COUNT(color) AS 'count color' FROM AUTO
WHERE MARK = 'LADA'
GROUP BY color, mark 
UNION SELECT MARK, color, COUNT(color) AS 'count color' FROM AUTO
WHERE MARK = 'BMW'
GROUP BY color, mark;
...
SELECT 
a1.MARK, 
(select count(*) from AUTO a2 where a2.MARK != a1.MARK) AS 'count cars' 
FROM AUTO a1
GROUP BY a1.MARK;
